        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          >
            <h1 className="text-5xl font-serif font-black text-slate-900 tracking-tight leading-none mb-3">
              Hello, {user?.userName?.split(" ")[0] || "Staff"}
            ...
            <Link key={stat.label} to={stat.to} className="group">
              ...
              >
                <div className="relative z-10">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 shadow-sm transition-transform group-hover:scale-110 ${stat.highlight ? "bg-white/30 backdrop-blur-md" : stat.bg}`}
                  >
                    <stat.icon
                      className={stat.highlight ? "text-slate-900" : stat.color}
                      size={26}
                    />
                  </div>
                  <div>
                    <p
                      className={`text-[11px] uppercase tracking-widest font-black mb-1.5 ${stat.highlight ? "text-amber-900/60" : "text-slate-400"}`}
                    >
                      {stat.label}
                    </p>
                    <h3
                      className={`text-3xl font-serif font-black ${stat.highlight ? "text-amber-900" : "text-slate-900"}`}
                    >
                      {stat.value}
                    </h3>
                  </div>
                ...
                <div
                  className={`absolute top-6 right-6 transition-opacity ${stat.highlight ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                >
                  <ArrowUpRight
                    size={18}
                    className={
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-1 space-y-8">
              ...
              >
                <div className="relative z-10">
                  ...
                  <p className="text-slate-400 text-sm mb-8 font-medium leading-relaxed">
                    ...
                    awaiting manual audit. Delay in processing may impact
                    platform liquidity.
                  </p>
                  <Link
                    to="/staff/kyc"
                    className="flex items-center justify-center gap-3 w-full py-4 bg-white text-slate-900 rounded-[1.25rem] text-xs font-black uppercase tracking-widest hover:bg-amber-50 transition-all shadow-xl"
                  >
                    Process Ledger
                    <ArrowRight size={16} />
                  </Link>
            ...
            <div className="bg-white p-8 rounded-[3rem] border border-slate-200/80 shadow-sm">
              <h4 className="text-[11px] uppercase font-black tracking-widest text-slate-400 mb-8">
                Administrative Hub
              </h4>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-[3rem] border border-slate-200/80 shadow-sm overflow-hidden flex flex-col h-full">
              <div className="p-8 lg:p-10 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10">
                ...
                <Link
                  to="/staff/transactions"
                  className="px-6 py-3 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl"
                >
                  Full Report
                </Link>
              </div>
                ...
                {loading ? (
                  ...
                  <div className="p-24 flex flex-col items-center justify-center gap-6 text-center">
                    ...
                    <p className="text-slate-400 font-medium">
                      No transactions recorded in this epoch.
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="flex items-center gap-6">
                          ...
                          >
                            <Coins
                              size={28}
                              className={
                                tx.asset === "GOLD" ? "fill-amber-500/20" : ""
                              }
                            />
                            <div
                              className={`absolute inset-0 bg-white/10 group-hover:bg-transparent transition-colors`}
                            />
                          </div>
                          <div>
                            <p className="font-black text-slate-900 group-hover:text-indigo-600 transition-colors text-[17px] tracking-tight">
                              {tx.user?.userName || "Anonymous"}
                            </p>
